...
def buildImage() {
    ...
    withCredentials([usernamePassword(credentialsId: 'docker-hub-repo', passwordVariable: 'PASS', usernameVariable: 'USER')]) {
        sh "docker build -t ahmedkhammassi/my-repo:spring-app-${IMAGE_NAME} ."
        sh "echo $PASS | docker login -u $USER --password-stdin"
        sh "docker push ahmedKlai/my-repo:spring-app-${IMAGE_NAME}"
    }
}
...
def sonarScan(String serverIp, String serverUser) {
    ...
    def runSonar = '"export MYSQLDB_ROOT_PASSWORD=ahmed MYSQLDB_DATABASE=pet_store MYSQLDB_LOCAL_PORT=3306 MYSQLDB_DOCKER_PORT=3306 && bash runSonarQube.sh"'
...
def deployApp(String serverIp, String serverUser) {
    ...
    def composeRun = '"export MYSQLDB_USER=root MYSQLDB_ROOT_PASSWORD=ahmed MYSQLDB_DATABASE=pet_store MYSQLDB_LOCAL_PORT=3306 MYSQLDB_DOCKER_PORT=3306 SPRING_LOCAL_PORT=8080 SPRING_DOCKER_PORT=8080 && docker-compose up -d"'
